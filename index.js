...
// ====================================
// FOLHAS DE TOM — dados de treino de voz/diálogo, um .md por folha, fora
// da pasta pública da extensão (essa aqui, __dirname, é a pasta do PLUGIN,
// que não é servida como estático pelo ST — diferente de EXTENSAO_DIR,
// que é). Guardar dado de RP privado num caminho estático seria expor por
// URL direta pra quem soubesse o nome do arquivo; aqui não tem esse risco.
// ====================================
const DADOS_DIR = path.join(__dirname, '_dados');
const TOM_DIR = path.join(DADOS_DIR, 'tom');
...
function pastaPersonagemTom(personagem) {
    return path.join(TOM_DIR, slugSeguro(personagem, 'personagem'));
}
function caminhoFolhaTom(personagem, slug) {
    const dirPersonagem = pastaPersonagemTom(personagem);
    ...
    if (path.dirname(resolvido) !== dirPersonagem) throw new Error('caminho da folha caiu fora da pasta esperada — recusado.');
    return resolvido;
}
function montarFrontmatter({ titulo, tags, criadoEm, atualizadoEm }) {
    const linhas = [
        '---',
...
// Chave de backup pra uma folha de tom — namespaced, nunca colide com os
// 3 nomes fixos da extensão (esses não têm "/" no nome).
function chaveTom(personagem, slug) {
    return 'tom__' + slugSeguro(personagem, 'personagem') + '__' + slugSeguro(slug, 'slug') + '.md';
}
...
module.exports = {
    ...
    init: async (router) => {
        ...
        // ====================================
        router.post('/tom/listar', async (req, res) => {
            try {
                const { personagem } = req.body || {};
                const dir = pastaPersonagemTom(personagem);
                ...
                for (const nome of arquivos.filter((n) => n.endsWith('.md'))) {
                    ...
                    folhas.push({ slug: nome.replace(/\.md$/, ''), titulo: meta.titulo || nome, tags: meta.tags || [], atualizadoEm: meta.atualizadoEm || null, bytes: Buffer.byteLength(bruto, 'utf8') });
                }
                folhas.sort((a, b) => String(b.atualizadoEm || '').localeCompare(String(a.atualizadoEm || '')));
                res.json({ personagem, folhas });
            } catch (e) {
                res.status(400).json({ erro: e.message });
            }
        });

        router.post('/tom/ler', async (req, res) => {
            try {
                const { personagem, slug } = req.body || {};
                const caminho = caminhoFolhaTom(personagem, slug);
                const bruto = await fs.readFile(caminho, 'utf8').catch((e) => {
                    if (e.code === 'ENOENT') throw new Error('folha "' + slug + '" não existe pra ' + personagem + '.');
                    throw e;
                });
                const { meta, corpo } = separarFrontmatter(bruto);
                res.json({ personagem, slug, titulo: meta.titulo || slug, tags: meta.tags || [], criadoEm: meta.criadoEm || null, atualizadoEm: meta.atualizadoEm || null, conteudo: corpo });
            } catch (e) {
                res.status(400).json({ erro: e.message });
            }
        });

        router.post('/tom/escrever', async (req, res) => {
            try {
                const { personagem, slug, titulo, tags, conteudo } = req.body || {};
                if (typeof conteudo !== 'string' || !conteudo.trim()) throw new Error('conteúdo vazio — recusado.');
                if (Buffer.byteLength(conteudo, 'utf8') > TAMANHO_MAX_BYTES) throw new Error('conteúdo maior que o teto de ' + (TAMANHO_MAX_BYTES / 1024 / 1024) + 'MB — recusado.');
                const dir = pastaPersonagemTom(personagem);
                await fs.mkdir(dir, { recursive: true });
                const caminho = caminhoFolhaTom(personagem, slug);
                const chave = chaveTom(personagem, slug);
                ...
                const texto = montarFrontmatter({ titulo: titulo || metaExistente.titulo || slug, tags: tags || metaExistente.tags || [], criadoEm: metaExistente.criadoEm, atualizadoEm: new Date().toISOString() }) + conteudo;
        ...
        router.post('/tom/apagar', async (req, res) => {
            try {
                const { personagem, slug } = req.body || {};
                const caminho = caminhoFolhaTom(personagem, slug);
                const chave = chaveTom(personagem, slug);
