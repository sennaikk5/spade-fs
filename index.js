...
function pastaPersonagemDocumentos(personagem, chatId) {
    const base = path.join(DOCUMENTOS_DIR, slugSeguro(personagem, 'personagem'));
    return chatId ? path.join(base, '_chats', slugSeguro(chatId, 'chatId')) : base;
}
function caminhoDocumento(personagem, slug, chatId) {
    const dirPersonagem = pastaPersonagemDocumentos(personagem, chatId);
...
function montarFrontmatter({ titulo, categoria, tags, criadoEm, atualizadoEm, sempreIncluir }) {
    const linhas = [
        '---',
        'titulo: ' + JSON.stringify(titulo || ''),
        'categoria: ' + JSON.stringify(categoria || 'geral'),
        'tags: ' + JSON.stringify(Array.isArray(tags) ? tags : []),
        'sempreIncluir: ' + JSON.stringify(!!sempreIncluir),
...
function chaveDocumento(personagem, slug, chatId) {
    return 'documento__' + slugSeguro(personagem, 'personagem') + (chatId ? '__chat-' + slugSeguro(chatId, 'chatId') : '') + '__' + slugSeguro(slug, 'slug') + '.md';
}
...
module.exports = {
    ...
    init: async (router) => {
        ...
        router.post('/documento/listar', async (req, res) => {
            try {
                const { personagem, chatId } = req.body || {};
                async function lerPasta(dir, escopo) {
                    ...
                    for (const nome of arquivos.filter((n) => n.endsWith('.md'))) {
                        ...
                        docs.push({ slug: nome.replace(/\.md$/, ''), titulo: meta.titulo || nome, categoria: meta.categoria || 'geral', tags: meta.tags || [], sempreIncluir: !!meta.sempreIncluir, atualizadoEm: meta.atualizadoEm || null, bytes: Buffer.byteLength(bruto, 'utf8'), escopo });
                    }
                    return docs;
                }
                let documentos = await lerPasta(pastaPersonagemDocumentos(personagem), 'personagem');
                if (chatId) documentos = documentos.concat(await lerPasta(pastaPersonagemDocumentos(personagem, chatId), 'chat'));
        ...
        router.post('/documento/ler', async (req, res) => {
            try {
                const { personagem, slug, chatId } = req.body || {};
                async function tentar(comChatId) {
                    const caminho = caminhoDocumento(personagem, slug, comChatId);
                    return fs.readFile(caminho, 'utf8');
                }
                let bruto = chatId ? await tentar(chatId).catch(() => null) : null;
                if (bruto == null) bruto = await tentar(null).catch((e) => {
        ...
        router.post('/documento/escrever', async (req, res) => {
            try {
                const { personagem, slug, titulo, categoria, tags, conteudo, chatId, sempreIncluir } = req.body || {};
                if (typeof conteudo !== 'string' || !conteudo.trim()) throw new Error('conteúdo vazio — recusado.');
                if (Buffer.byteLength(conteudo, 'utf8') > TAMANHO_MAX_BYTES) throw new Error('conteúdo maior que o teto de ' + (TAMANHO_MAX_BYTES / 1024 / 1024) + 'MB — recusado.');
                const dir = pastaPersonagemDocumentos(personagem, chatId);
                await fs.mkdir(dir, { recursive: true });
                const caminho = caminhoDocumento(personagem, slug, chatId);
                const chave = chaveDocumento(personagem, slug, chatId);
                ...
                const texto = montarFrontmatter({ titulo: titulo || metaExistente.titulo || slug, categoria: categoria || metaExistente.categoria || 'geral', tags: tags || metaExistente.tags || [], sempreIncluir: sempreIncluir !== undefined ? sempreIncluir : metaExistente.sempreIncluir, criadoEm: metaExistente.criadoEm, atualizadoEm: new Date().toISOString() }) + conteudo;
        ...
        router.post('/documento/apagar', async (req, res) => {
            try {
                const { personagem, slug, chatId } = req.body || {};
                async function existeEm(comChatId) {
                    try { await fs.access(caminhoDocumento(personagem, slug, comChatId)); return true; } catch { return false; }
                }
                const alvoChatId = (chatId && await existeEm(chatId)) ? chatId : null;
                const caminho = caminhoDocumento(personagem, slug, alvoChatId);
                const chave = chaveDocumento(personagem, slug, alvoChatId);
